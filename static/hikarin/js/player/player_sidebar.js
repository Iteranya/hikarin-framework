export class PlayerSidebar {
    constructor(gameRunner) {
        ...
        // Only render if DOM exists
        if (this.dom.content) this.renderContent();
    }
    ...
    renderContent() {
        ...
        this.dom.btnReset = document.getElementById('ps-btn-reset');
        this.dom.btnEnv = document.getElementById('ps-btn-env');
        ...
        if (this.dom.btnPlay) this.dom.btnPlay.addEventListener('click', () => this.handlePlay());
        if (this.dom.btnStop) this.dom.btnStop.addEventListener('click', () => this.handleStop());
        if (this.dom.btnReset) this.dom.btnReset.addEventListener('click', () => this.handleReset());
        if (this.dom.btnEnv) this.dom.btnEnv.addEventListener('click', () => this.handleToggleEnvironment());
        if (this.dom.btnClearLog) this.dom.btnClearLog.addEventListener('click', () => this.handleClearLogs());
    }
    ...
    renderLogs() {
        ...
        const categoryColors = {
            STEP: "text-cyan-400", FLOW: "text-green-400", VAR: "text-orange-400",
            COND: "text-purple-400", ERR: "text-red-500 font-bold", WARN: "text-yellow-400",
        };
        ...
        const html = this.logs.map(entry => {
            ...
            const argsString = entry.args.length > 0
                ? ` <span class="text-gray-500">${JSON.stringify(entry.args)}</span>` : '';
    ...
    setRunningState(running) {
        ...
        if (running) {
            ...
            this.toggleBtn(this.dom.btnEnv, true, 'bg-blue-700');
    ...
    updateStatus(text) {
        if (this.dom.status) this.dom.status.innerText = `State: ${text}`;
    }
    ...
    close() {
        if (this.dom.sidebar) { this.dom.sidebar.classList.add('hidden'); this.dom.sidebar.classList.remove('flex'); }
        if (this.dom.button) { this.dom.button.classList.remove('bg-blue-600', 'text-white'); this.dom.button.classList.add('bg-gray-700'); }
    }
    renderState() {
        if (!this.dom.sidebar) return;
        if (this.isOpen) {
            this.dom.sidebar.classList.remove('hidden'); this.dom.sidebar.classList.add('flex');
            if (this.dom.button) { this.dom.button.classList.add('bg-blue-600', 'text-white'); this.dom.button.classList.remove('bg-gray-700'); }
        } else {
            this.dom.sidebar.classList.add('hidden'); this.dom.sidebar.classList.remove('flex');
            if (this.dom.button) { this.dom.button.classList.remove('bg-blue-600', 'text-white'); this.dom.button.classList.add('bg-gray-700'); }
        }
    }
    showButton() { if (this.dom.button) this.dom.button.classList.remove('hidden'); }
    hideButton() { if (this.dom.button) this.dom.button.classList.add('hidden'); }
}
