...
export class FlowEditorManager {
  constructor(fileManager) {
    ...
    this.activeSceneId = null;               // ← the active scene id

    // We'll attach _openFormEditor from main.js later
    this._openFormEditor = null;
  ...
  _switchToTab(tab) {
    ...
    document.querySelectorAll('#flow-editor-tabs .flow-editor-tab').forEach(t => {
      t.style.color = '#6b7280';
      t.style.borderBottomColor = 'transparent';
    });
    const activeBtn = document.getElementById(`tab-panel-${tab}`);
    if (activeBtn) {
      activeBtn.style.color = '#22c55e';
      activeBtn.style.borderBottomColor = '#22c55e';
    }

    // Hide all content panels
    document.getElementById('flow-editor-content').style.display = 'none';
    document.getElementById('flow-script-content').style.display = 'none';
    document.getElementById('flow-ace-container').style.display = 'none';

    if (tab === 'flow') {
      document.getElementById('flow-editor-content').style.display = '';
      document.getElementById('flow-editor-title').textContent = 'Flow';
    } else if (tab === 'script') {
      document.getElementById('flow-script-content').style.display = 'flex';
      document.getElementById('flow-editor-title').textContent = 'Script';
      if (this.activeSceneId && this._openFormEditor) {
        this._openFormEditor(this, this.activeSceneId);
      }
    } else if (tab === 'code') {
      document.getElementById('flow-ace-container').style.display = 'flex';
      document.getElementById('flow-editor-title').textContent = 'Code';
      if (this.activeSceneId) {
        openCodeTab(this, this.activeSceneId);
      }
    }
  }
  ...
  return `
    ...
    <div id="flow-editor-panel" class="hidden glass-panel z-20 flex-col border-l border-gray-800" style="width:360px;min-width:280px;max-width:600px;position:relative;">
      ...
      <div style="display:flex;align-items:center;border-bottom:1px solid #374151;background:#1a1a1a;flex-shrink:0;" id="flow-editor-tabs">
        <div class="flow-editor-tab active" id="tab-panel-flow" data-tab="flow"
             style="padding:0.4rem 1rem;font-size:0.7rem;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;color:#22c55e;cursor:pointer;border-bottom:2px solid #22c55e;">Flow</div>
        <div class="flow-editor-tab" id="tab-panel-script" data-tab="script"
             style="padding:0.4rem 1rem;font-size:0.7rem;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;color:#6b7280;cursor:pointer;border-bottom:2px solid transparent;">Script</div>
        <div class="flow-editor-tab" id="tab-panel-code" data-tab="code"
             style="padding:0.4rem 1rem;font-size:0.7rem;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;color:#6b7280;cursor:pointer;border-bottom:2px solid transparent;">Code</div>
      ...
      <div style="position:relative;flex:1;min-height:200px;overflow:hidden;">
        ...
        <!-- Script tab (form editor) -->
        <div id="flow-script-content" style="position:absolute;top:0;left:0;right:0;bottom:0;display:none;flex-direction:column;background:#121212;">
          <div id="form-toolbar" class="p-2 border-b border-gray-800 flex flex-wrap gap-2 flex-shrink-0" style="background:#1e1e1e;">
            <button data-type="say" class="form-add-btn">💬 Say</button>
            <button data-type="show" class="form-add-btn">🖼 Show</button>
            <button data-type="remove" class="form-add-btn">❌ Remove</button>
            <button data-type="background" class="form-add-btn">🏞 Bg</button>
            <button data-type="play_music" class="form-add-btn">🎵 Music</button>
            <button data-type="stop_music" class="form-add-btn">🔇 Stop</button>
            <button data-type="set_var" class="form-add-btn">📊 Set Var</button>
            <button data-type="mod_var" class="form-add-btn">🔧 Mod Var</button>
            <button data-type="cond_same" class="form-add-btn">🔀 If Equal</button>
            <button data-type="cond_more" class="form-add-btn">🔀 If ></button>
            <button data-type="cond_less" class="form-add-btn">🔀 If <</button>
          </div>
          <div id="flow-script-blocks" class="flex-1 overflow-y-auto p-4 space-y-3"></div>
          <div class="flex items-center justify-between p-2 border-t border-gray-800 flex-shrink-0 text-xs bg-dark-950/40">
            <span id="form-status" class="text-gray-500">Ready</span>
            <button id="btn-form-save" class="px-3 py-1 bg-mob-600 text-white rounded text-xs hover:bg-mob-500 transition font-medium">
              Save Script
            </button>
          </div>
        </div>
        ...
        <!-- Code tab (ACE editor) -->
        <div id="flow-ace-container" style="position:absolute;top:0;left:0;right:0;bottom:0;display:none;flex-direction:column;background:#121212;">
          <div class="flex items-center p-3 border-b border-gray-800 flex-shrink-0" style="background:#1e1e1e;">
            <i class="fa-brands fa-python text-gray-500 mr-2"></i>
            <h2 id="flow-code-filename" class="text-sm font-bold text-white">scene.py</h2>
          </div>
          <div id="flow-ace-wrapper" class="flex-1 min-h-0">
            <div id="flow-ace-editor" class="h-full w-full"></div>
          </div>
          <div style="height:28px;display:flex;align-items:center;justify-content:space-between;padding:0 12px;background:#0a0a0a;border-top:1px solid rgba(255,255,255,0.05);flex-shrink:0;">
            <span id="flow-code-filename-bottom" style="font-size:10px;color:#6b7280;font-family:'Fira Code',monospace;"></span>
...
FlowEditorManager.prototype._bindEvents = function() {
  ...
  // Tab buttons
  document.getElementById('tab-panel-flow')?.addEventListener('click', () => this._switchToTab('flow'));
  document.getElementById('tab-panel-script')?.addEventListener('click', () => this._switchToTab('script'));
  document.getElementById('tab-panel-code')?.addEventListener('click', () => this._switchToTab('code'));
  ...
  // Tab switching (old code will be replaced by the new tabs, so we remove this)
  /* The old tab switching that called openCodeTab is now handled by _switchToTab.
     We'll remove the old code that listened on #flow-editor-tabs to avoid conflicts. */
  // We'll still keep the context menu and pan/zoom bindings below.
  // ─── Context menu ──
